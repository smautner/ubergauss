...
class nutype:
    ...
    def __init__(self, space, f, data, numsample = 16):
        ...
        self.samplers = make_samplers( space)
        self.scores = []
    ...
    def nuParams(self):
        '''
        we should check who is responsible for our gains
        - topscores connect?  -> good
        - for categorical, if i sampled 3 and they are below median?, i should kick them

        then we keep the topscorers of the df

        when resamping
            - if changed: resample from new interval
            - else: resample +x, delete those close to the values from the old thing
        '''

        # what we want to analyse is the ok part from last time plus the new runs:
        data = pd.concat((self.df , self.carry))
        # these are the interesting columns
        col_names = [col for col in self.df.columns if col not in ['time', 'score', 'datafield']]

        needs_update = self.check_responsibility(data,col_names) # -> dict {key: rmthese or 1}
        self.NU = needs_update


        cnum = int(self.numsample*.4)
        self.carry = self.df.sort_values(by='score', ascending=False).head(cnum)

        d = {}
        for a in col_names:
            if a in needs_update:
                d[a]  = self.samplers[a].sample_learn(data.score, data[a].tolist(),self.numsample, needs_update[a])
            else:
                d[a]  = self.samplers[a].sample_avoid( self.carry[a].tolist(),self.numsample)
    ...
    def check_responsibility(self, data, col_names):
        r = {}
        for e in col_names:
            col =  data[e].tolist()
            if isinstance(col[0], int):
                # check if any of the values are always below the median
                median_score = data.score.median()
                unique_values = data[e].unique()
                failed = []
                for val in unique_values:
                    val_scores = data[data[e] == val]['score']
                    if len(val_scores) >= 3 and (val_scores < median_score).all():
                        failed.append(val)
                if failed:
                    r[e] = failed
            else:
                score =need_sampler(data.score, col)
                if score < 0.5:
                    r[e] = 1
        return r
...
def make_samplers( spaceship) -> dict:
    params = hypersample(spaceship.space)
    samplers = {}
...
class intsampler():
    ...
    def sample(self, scores, values, numsample):
        return [self.sampler() for _ in numsample]

    def sample_learn(self, scores, values : list , numsample: int, notthis: list):
        current_unique_values = list(set(values))
        valid_values_to_sample_from = [v for v in current_unique_values if v not in notthis]
        self.sampler = lambda: random.choice(valid_values_to_sample_from)
        return [self.sampler() for _ in range(numsample)]

    def sample_avoid(self,  carryval, numsample):
        s =  [self.sampler() for _ in range(numsample+len(carryval))]
        for e in carryval:
            if e in s:
                s.remove(e)
        random.shuffle(s)
...
class floatsampler():
    ...
    def sample_learn(self, scores, values, numsample, asd):
        self.sampler = learn_float_sampler(scores,values)
        return [self.sampler() for _ in range(numsample)]


    def sample_avoid(self, values, numsample):
        # just sample, n+len(carry), then delete values close to carry
        samples = [self.sampler() for _ in range(numsample+len(values))]

        # remove stuff close to what we have, so next round things are uniformish
        distances = np.abs(np.subtract.outer(samples, values))
        min_distances = np.min(distances, axis=1)
        closest_indices = np.argsort(min_distances)[:len(values)]
        all_indices = np.arange(len(samples))
        keep_indices = np.delete(all_indices, closest_indices)
